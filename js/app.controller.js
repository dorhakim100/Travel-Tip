...
function onOpenRemoveDialog(locId) {
...
function renderRemoveDialog(locId) {
  const elDialog = document.querySelector('dialog')
  locService.getById(locId).then((loc) => {
    console.log(loc.name)
    elDialog.innerHTML = `
        <h2>Are you sure?</h2>
        <p>Removing ${loc.name}</p>
        <form method="dialog">
            <div class="dialog-btn-container">
    
                <button onclick="app.onCloseDialog()">Cancel</button>
                <button data-id="${locId}" onclick="app.removeLoc(this)">Remove</button>
...
function removeLoc(elRemove) {
  const locId = elRemove.dataset.id
  locService
    .remove(locId)
    .then(() => {
